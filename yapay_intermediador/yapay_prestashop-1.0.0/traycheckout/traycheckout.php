...
class traycheckout extends PaymentModule
{
	...
	public function __construct()
    {
        ...
        $this->displayName 		= $this->l('Yapay Intermediador');
        $this->description 		= $this->l('Aceitar pagamentos via Yapay Intermediador');
		$this->confirmUninstall = $this->l('Tem certeza de que pretende eliminar os seus dados?');
		$this->textshowemail 	= $this->l('Você deve seguir corretamente os procedimentos de pagamento do Yapay Intermediador, para que sua compra seja validada.');
	}
	...
	public function create_states()
	{
		...
		array( 'RoyalBlue', '00100', 'Yapay Intermediador - Transação em Andamento', '' ),
		array( 'LimeGreen', '11110', 'Yapay Intermediador - Transação Aprovada','payment' ),
		array( 'Crimson',   '01110', 'Yapay Intermediador - Transação Cancelada', 'order_canceled'	),
		array( '#8f0621',   '00100', 'Yapay Intermediador - Recuperação de Pagamento', 'payment_error' ),
		array( '#ec2e15',   '00100', 'Yapay Intermediador - Contestação de Pagamento', 'refund' )
		);
	...
	{
		$this->_html = '<h2>Yapay Intermediador</h2>';
		
		if (isset ( $_POST ['submittraycheckout'] )) {

			if (empty ( $_POST ['token_store'] ))
				$this->_postErrors [] = $this->l ( 'Digite o token para utilizar o módulo de pagamento Yapay Intermediador' );
	...
	public function displayFormSettings()
	{
		...
		$arrContent = array(
			'msg_aceite_traycheckout'=>$this->l('Este módulo permite aceitar pagamentos via Yapay Intermediador'),
			'msg_cliente_escolher_traycheckout'=>$this->l('Se o cliente escolher o módulo de pagamento, a conta do Yapay Intermediador será automaticamente creditado'),
			'msg_configurar_email'=>$this->l('Você precisa configurar o seu token do Yapay Intermediador, para depois usar este módulo'),
    ...
    public function __execEPayment($cart)
    {
